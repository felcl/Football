...
function NFT() {
  ...
  return (
    <div>
      ...
      {userCard.length > 0 && (
        <CardDetails
          isShow={showCardDetail}
          showMerge={showMergeFun}
          showCreateOrder={createOrderFun}
          CardInfo={userCard[cardDetialIndex]}
          close={() => setShowCardDetail(false)}
          type="NFT"
        ></CardDetails>
      )}
      {/* 卡牌挂卖 */}
      {userCard.length > 0 && (
        <CardDetails
          isShow={showCreateOrder}
          CardInfo={userCard[cardDetialIndex]}
          close={() => setShowCreateOrder(false)}
          type="CreateOrder"
        ></CardDetails>
      )}
      {/* 卡牌合成 */}
      <CardSynthesis
        isShow={showCardSynthesis}
        CardInfo={userCard[cardDetialIndex]}
        close={() => setshowCardSynthesis(false)}
      ></CardSynthesis>
      <div className="Edition-Center">
        <div className="SwapTitle">NFT - 庫存</div>
        {/* 盲盒开启 */}
        <BoxOpen></BoxOpen>
        {/* 卡牌合成规则 */}
        ...
        <div className="screen">
          <div className="Tabs">
            <div
              className={
                TabIndex === 0 ? "activeTab linear-gradient" : "invalidTab"
              }
              onClick={() => {
                SetTabIndex(0);
              }}
            ...
            <div
              className={
                TabIndex === 1 ? "activeTab linear-gradient" : "invalidTab"
              }
              onClick={() => {
                SetTabIndex(1);
              }}
